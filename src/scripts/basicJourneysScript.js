...
const submitRevBodyEditCheck = scriptData[2].submitRevBodyEditCheck;
...
export const options = {
  ...
  thresholds: {
    ...
    load_iras_home_page_response_time: ["p(100)<3000", "p(95)<1500"],
  },
  scenarios: {
    BasicJourneysScript: {
...
export function basicJourneysScript(data) {
  ...
  group("Manage Review Body Journey", function () {
    ...
    //   "Submit Review Body Page Request Success": () =>
    //     response.status === 200,
    ...
    const isPostSubmitRevBodyEditPageReqSuccessful = check(response, {
      "Submit Review Body Edit Request Success": () => response.status === 302,
    });
    console.info(
      "Request Sent: " + response.request.method + " " + response.request.url
    );
    if (!isPostSubmitRevBodyEditPageReqSuccessful) {
      console.error(
