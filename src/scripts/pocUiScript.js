...
export const TrendHomePageReqDuration = new Trend(
  "load_k6_home_page_response_time",
  true
);
export const TrendContactPageReqDuration = new Trend(
  "load_k6_contact_page_response_time",
  true
);
export const TrendNewsPageReqDuration = new Trend(
  "load_k6_news_page_response_time",
  true
);
export const TrendMessagesPageReqDuration = new Trend(
  "load_k6_msg_page_response_time",
  true
);
export const TrendMessagesLoginReqDuration = new Trend(
  "login_k6_msg_page_response_time",
  true
);
...
export function pocK6UiJourney() {
  ...
  group("Load k6 Contact Page User Journey", function () {
    ...
    if (!isHomePageReqSuccessful) {
      console.error(
        `Load k6 Home Page Request Failed - ${response.url} \nStatus - ${response.status}` +
    ...
    const isContactPageReqSuccessful = check(response, {
      ...
      "Contact Page Request Time < 5secs": () =>
        response.timings.duration < 5000,
      "Contact Page Request Time < 1secs": () =>
    ...
    if (!isContactPageReqSuccessful) {
      console.error(
        `Load k6 Contact Page Request Failed - ${response.url} \nStatus - ${response.status}` +
          `\nResponse Time - ${response.timings.duration} \nError Code - ${response.error_code}`
      );
    }
    userThinkTime();
  ...
  group("Load k6 News Page User Journey", function () {
    ...
    if (!isNewsPageReqSuccessful) {
      console.error(
        `Load k6 News Page Request Failed - ${response.url} \nStatus - ${response.status}` +
          `\nResponse Time - ${response.timings.duration} \nError Code - ${response.error_code}`
      );
    }
  });
  ...
  group("Load k6 and Login to Message", function () {
    ...
    const isMessagesPageReqSuccessful = check(response, {
      "Unauthorized Messages Page Request Success": () =>
        response.status === 200,
      "Unauthorized Messages Page Request Time < 5secs": () =>
        response.timings.duration < 5000,
      "Unauthorized Messages Page Request Time < 1secs": () =>
        response.timings.duration < 1000,
    });
    console.log(
      "Request Sent: " + response.request.method + " " + response.url
    ...
    if (!isMessagesPageReqSuccessful) {
      console.error(
        `Load k6 Unauthorized Messages Page Request Failed - ${response.url} \nStatus - ${response.status}` +
          `\nResponse Time - ${response.timings.duration} \nError Code - ${response.error_code}`
      );
    }
    ...
    const isMessagePageUnauthorized = check(response, {
      "Unauthorized Messages Header Present": (r) =>
    ...
    if (!isMessagePageUnauthorized) {
      ...
      response = http.post(`${baseURL}/login.php`, {
        login: `${userValues[0].username}`,
        password: `${userValues[0].password}`,
        redir: "1",
        csrftoken: `${token}`,
      });
      ...
      const isMessageLoginReqSuccessful = check(response, {
        ...
        "Authorized Messages Header Present": (r) =>
          r.body.indexOf("Welcome, admin!") !== -1,
        "Login Messages Page Request Time < 5secs": () =>
          response.timings.duration < 5000,
        "Login Messages Page Request Time < 1secs": () =>
          response.timings.duration < 1000,
      });
      console.log(
        "Request Sent: " + response.request.method + " " + response.url
      ...
      if (!isMessageLoginReqSuccessful) {
        console.error(
          `Load k6 Authorized Messages Page Request Failed - ${response.url} \nStatus - ${response.status}` +
            `\nResponse Time - ${response.timings.duration} \nError Code - ${response.error_code}`
        );
      }
    }
