...
//Custom Metric
export const TrendPostPetReqDuration = new Trend('post_pet_response_time', true);
export const TrendGetPetReqDuration = new Trend('get_pet_response_time', true);
export const TrendDeletePetReqDuration = new Trend('delete_pet_response_time', true);
...
export function pocK6ApiJourney() {
    ...
    group('Create, Fetch & Remove Pet Journey', function () {
        ...
        if (!isPostPetReqSuccessful) {
            console.error(`Post Pet Request Failed - ${response.url} \nStatus - ${response.status}` +
        ...
        const isGetPetReqSuccessful = check(response,
            {
                ...
                'Get Pet Request Expected Pet Id Returned': (r) => r.body.includes(`"id":${petId}`) == true,
        ...
        if (!isGetPetReqSuccessful) {
            console.error(`Get Pet Request Failed - ${response.url} \nStatus - ${response.status}` +
        ...
        const isDeletePetReqSuccessful = check(response,
            {
                ...
                'Delete Pet Request Expected Pet Id Returned': (r) => r.body.includes(`"message":"${petId}"`) == true,
        ...
        if (!isDeletePetReqSuccessful) {
            console.error(`Delete Pet Request Failed - ${response.url} \nStatus - ${response.status}` +
                `\nResponse Time - ${response.timings.duration} \nError Code - ${response.error_code} \nResponse Body - ${response.body}`);
        };
        userThinkTime();
    });
...
export function handleSummary(data) {
    return {
        ...
        'results/report.json': JSON.stringify(data)
};
}