...
export async function decryptData(textToDecrypt) {
  const encodedData = base64Decode(textToDecrypt);
  //Key must match the one used to encrypt
  const key = new Uint8Array(keyValues);

  try {
    return await decrypt(key, encodedData);
  } catch (e) {
    console.log("Error: " + JSON.stringify(e));
  }
}

const decrypt = async (key, encodedData) => {
  // Extract Initialization Vector
  const initializeVectorLength = 12;
  const iv = new Uint8Array(encodedData.subarray(0, initializeVectorLength));

  // Extract Encrypted Data
  const encryptedData = new Uint8Array(
    encodedData.subarray(initializeVectorLength)
  );
